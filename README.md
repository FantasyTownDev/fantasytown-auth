...
Minecraft 第三方验证登录系统最小化内核模块，基于 .NET 10 构建。

## 一句话定位

一个最小化的 Minecraft 第三方验证登录系统内核：基于 Yggdrasil 协议实现外置登录，包含皮肤服务（材质签名分发、全服可见），采用垂直切片模块化单体架构，允许后续扩展绑定正版账号。

## 功能特性

- **Yggdrasil 外置登录**：完整协议支持（authenticate/validate/refresh/invalidate/signout/join/hasJoined/profile）
- **皮肤服务**：材质上传、RSA 签名、预计算缓存、全服可见
